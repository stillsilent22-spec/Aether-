...
pub mod engine_flags {
    ...
    pub const ANCHOR: u64      = 1 << 3;  // [3] pi / phi / sqrt2 / e Detektion
    ...
    pub const NOETHER: u64     = SYMMETRY;
    pub const HEISENBERG: u64  = TRUST;
    pub const MANDELBROT: u64  = PERIODICITY;
    pub const FOURIER: u64     = PERIODICITY;
    pub const BENFORD: u64     = ANCHOR;
    pub const ALL_CONFIRMED: u64 = 0x3FF; // alle 10 Filter bestätigt
}
...
impl EnginePipeline {
    ...
    pub(crate) fn evaluate(
        ...
        flags |= engine_flags::SECURITY;
        // [1] Shannon: gültige Entropie vorhanden
        if entropy > 0.0 && entropy <= 8.0 {
            flags |= engine_flags::SHANNON;
        }
        // [2] H_lambda: Restunsicherheit unter Heisenberg-Grenze
        if h_lambda <= 2.0 {
            flags |= engine_flags::H_LAMBDA;
        }
        // [3] Anchor: Konstantennaehe (pi/phi/e/sqrt2) + Benford
        let anchor_score = structural_constant_score(original);
        if anchor_score >= 0.25 || benford >= 0.30 {
            flags |= engine_flags::ANCHOR;
        }
        // [4] Symmetry (Noether): stabile Verteilungssymmetrie
        if symmetry >= 0.55 {
            flags |= engine_flags::SYMMETRY;
        }
        // [5] Delta: wird bei Encode/Decode gesetzt (hier Platzhalter)
        if delta_ratio < 0.80 {
            flags |= engine_flags::DELTA;
        }
        // [6] Periodicity (Mandelbrot/Fourier): strukturelle Wiederkehr
        ...
        // Symmetriebruch * Kohaerenz-Ueberraschung * Integritaetsphase
        let symmetry_break = (1.0_f32 - symmetry).clamp(0.0, 1.0);
        let entropy_norm = (entropy / 8.0_f32).clamp(0.0, 1.0);
        let coherence_surprise = entropy_norm * (coherence_index as f32).clamp(0.0, 1.0);
        let e_lambda = ((0.50 * symmetry_break) + (0.50 * coherence_surprise)).clamp(0.0, 1.0);
        let e_lambda_label = if e_lambda < 0.15 { "LATENT" }
            else if e_lambda < 0.35 { "EMERGING" }
            else if e_lambda < 0.60 { "ACTIVE" }
            else { "CRITICAL" };
        ...
        EngineEvaluation {
            ...
            engine_flags: flags,
            e_lambda,
            e_lambda_label: e_lambda_label.to_string(),
        }
    }
...
impl AefEncoder {
    pub fn encode_sync(
        ...
        Ok(AefEncodeResult {
            ...
            e_lambda: file.trust_metadata.e_lambda,
            e_lambda_label: file.trust_metadata.e_lambda_label.clone(),
        })
    }
...
fn readable_engine_flags(flags: u64) -> Vec<String> {
    ...
    for (mask, label) in [
        (engine_flags::SECURITY,    "[0] Security"),
        (engine_flags::SHANNON,     "[1] Shannon"),
        (engine_flags::H_LAMBDA,    "[2] H_lambda"),
        (engine_flags::ANCHOR,      "[3] Anchor"),
...
/// Filter [3] Anchor: Berechnet Naehe zu strukturellen Konstanten (pi, phi, e, sqrt2).
/// Gibt Score 0.0–1.0: 1.0 = exakte Konstante, 0.0 = keine Naehe.
fn structural_constant_score(data: &[u8]) -> f32 {
    const CONSTANTS: [(&str, f32); 7] = [
        ("pi",    std::f32::consts::PI),
        ("phi",   1.618_033_9_f32),
        ("e",     std::f32::consts::E),
        ("sqrt2", std::f32::consts::SQRT_2),
        ("sqrt3", 1.732_050_8_f32),
        ("log2",  std::f32::consts::LN_2),
        ("ln10",  std::f32::consts::LN_10),
    ];
    if data.len() < 4 {
        return 0.0;
    }
    // Entropie als Float in [0,8] — wie nah ist sie an einer Konstante?
    let entropy = shannon_entropy(data);
    let mut best_score: f32 = 0.0;
    for (_name, constant) in &CONSTANTS {
        let deviation = (entropy - constant).abs() / constant;
        let score = (1.0 - deviation.clamp(0.0, 1.0)).max(0.0);
        if score > best_score {
            best_score = score;
        }
        // Auch normierte Symmetrie prüfen
        let symmetry = mirrored_symmetry(data);
        let dev_sym = (symmetry - constant / 4.0).abs();
        let score_sym = (1.0 - dev_sym.clamp(0.0, 1.0)).max(0.0);
        if score_sym > best_score {

...
pub fn render_reply(input: Option<&ShanwayInput>, user_text: &str) -> String {
    ...
    format!(
        "[Analyse] Datei {file_name} ({file_type}) | H_lambda {h_lambda:.3} | E_lambda {e_lambda:.3} ({e_lambda_label}) | I_obs {i_obs:.3} | Entropie {entropy:.3} | Wissen {knowledge:.1}% | Symmetrie {symmetry:.1}% | Delta-Pfade {delta_paths} | Boundary {boundary}. Noether, Mandelbrot, Heisenberg und Bayes wurden gemeinsam ausgewertet.\n\
...
[Status] Vault-first aktiv | keine Halluzination | strukturell, nicht semantisch.",
        ...
        e_lambda = file.e_lambda,
        e_lambda_label = file.e_lambda_label,

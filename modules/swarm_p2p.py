...
DEFAULT_P2P: Dict[str, Any] = {
    ...
    # Optional internet relay endpoint for peer discovery bridge
    "relay_url": "",
}
...
class P2PLayer:
    ...
    def publish_fingerprints(
        ...
        if self._transport is not None:
            try:
                relay_url = str(self._config.get("relay_url", "") or "").strip()
                if relay_url:
                    self._transport.relay_push(relay_url)
                return True
    ...
    def receive_gossip(self, msg: Dict[str, Any]) -> None:
        ...
        if incoming_node_id and incoming_device_fp:
            try:

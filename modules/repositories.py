...
import secrets
from .local_secret_store import protect_local_secret, unprotect_local_secret, is_protected_local_secret
...
class UserRepository:
    ...
    def _new_sync_materials() -> tuple[str, str]:
        """Erzeugt von Login-Daten getrennte Sync-Identitaeten."""
        return secrets.token_hex(12), secrets.token_hex(32)

    @staticmethod
    def _protect_local_secret(secret: str) -> str:
        """Schuetzt ein Secret fuer lokale Persistenz."""
        return protect_local_secret(secret)
    ...
    def _ensure_user_sync_material(
        ...
        derived_identity = hashlib.sha256(
            f"sync-identity|{username}|{password_hash}|{salt_hex}".encode("utf-8")
        ).hexdigest()[:24]
        derived_secret = hashlib.blake2b(
            f"sync-secret|{username}|{password_hash}|{salt_hex}".encode("utf-8"),
            digest_size=32,
        ).hexdigest()
        protected = protect_local_secret(derived_secret)
        self.connection.execute(
            "UPDATE users SET sync_identity = ?, sync_secret = ? WHERE id = ?",
            (derived_identity, protected, int(user_id)),
        )
        self.connection.commit()
        return derived_identity, derived_secret

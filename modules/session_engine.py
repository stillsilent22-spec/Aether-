...
class SessionContext:
    ...
    def noise_from_seed(seed: int, length: int) -> bytes:
        """
        Erzeugt kryptographisch sicheres Rauschen aus einem Session-Seed.

        Nutzt HKDF-Expand (HMAC-SHA256) statt random.Random.
        Deterministisch fuer gleichen Seed, aber kryptographisch stark:
        seed allein reicht nicht zur Rekonstruktion ohne HKDF-Kontext.
        """
        import hashlib as _hl
        import hmac as _hmac
        if length <= 0:
            return b""
        ikm = str(int(seed) & 0xFFFFFFFF).encode("ascii")
        salt = b"aether-noise-salt-v2"
        info = b"aether-session-noise-v2"
        # PRK via HKDF-Extract
        prk = _hl.pbkdf2_hmac("sha256", ikm, salt, 1, dklen=32)
        # HKDF-Expand
        chunks: list[bytes] = []
        prev = b""
        idx = 0
        ...
        while total < length:
            idx += 1
            prev = _hmac.new(prk, prev + info + bytes([idx]), "sha256").digest()
            chunks.append(prev)
            total += len(prev)
    ...
    def generate_aether_noise(self, length: int) -> bytes:
        """
        Erzeugt aus dem Session-Seed kryptographisch sicheres Rauschen.
        """

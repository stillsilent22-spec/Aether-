...
- [3] Anchor:      pi / phi / sqrt2 / e Detektion — Konstantennaehe als Invariante.
...
class ShanwayEngine:
    ...
    # Filter [3] Anchor: dieselben Konstanten wie analysis_engine._STRUCTURAL_CONSTANTS
    CONSTANTS = {
        "pi":    math.pi,
    ...
    def generate_output(self, assessment: ShanwayAssessment, assistant_text: str = "") -> str:
        ...
        # E_lambda Emergenz-Signal aus miniature_reflection lesen
        e_lambda_val = float(miniature.get("e_lambda", 0.0) or 0.0)
        e_lambda_label_val = str(miniature.get("e_lambda_label", "LATENT") or "LATENT")
        if e_lambda_val >= 0.15:
            sections.append(
                f"[Emergenz] E_lambda {e_lambda_val:.3f} | {e_lambda_label_val} | "
    ...
    def _append_structural_notes(self, response: str, assessment: ShanwayAssessment) -> str:
        ...
        notes.append(
            "FILTER[0-9]: "
            f"Security ✓ | "
            f"Shannon[{filter_trace['noether']}] | "
            f"H_lambda[{filter_trace['heisenberg']}] | "
            f"Anchor[{filter_trace['bayes']}] | "

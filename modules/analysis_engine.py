...
# Mathematische Referenzkonstanten fuer Filter [3] Anchor
_STRUCTURAL_CONSTANTS: dict[str, float] = {
    "pi":     math.pi,           # 3.14159...
    "phi":    (1.0 + math.sqrt(5.0)) / 2.0,  # 1.61803... Goldener Schnitt
    "e":      math.e,            # 2.71828...
    "sqrt2":  math.sqrt(2.0),    # 1.41421...
    "sqrt3":  math.sqrt(3.0),    # 1.73205...
...
def _nearest_structural_constant(value: float) -> tuple[str, float, float]:
    """
    Berechnet fuer einen gegebenen Wert die naechste mathematische Konstante.

    Returns: (name, constant_value, deviation)
    deviation = |value - constant| / constant  (normiert, 0 = exakt)
    """
    ...
    for name, const in _STRUCTURAL_CONSTANTS.items():
        # Direkte Naehe
        dev = abs(float(value) - const) / const
        if dev < best_deviation:
            best_deviation = dev
            best_name = name
            best_value = const
        # Normiert auf [0,1]: oft sind Werte als Bruchteil einer Konstante sichtbar
        if abs(float(value)) > 1e-12:
            ratio = const / abs(float(value))
            if 0.5 <= ratio <= 2.0:
                dev_ratio = abs(ratio - round(ratio)) / max(1.0, abs(round(ratio)))
                if dev_ratio < best_deviation:
                    best_deviation = dev_ratio
                    best_name = f"{name}_ratio"
                    best_value = const
    return best_name, best_value, round(float(min(1.0, best_deviation)), 6)
...
class AnalysisEngine:
    ...
    def _build_scan_anchor_entries(
        ...
        # Filter [3] Anchor: Konstantennaehe fuer jeden Eintrag berechnen
        for entry in raw_entries:
            val = float(entry.get("value", 0.0) or 0.0)
            name, const_val, deviation = _nearest_structural_constant(val)
            entry["nearest_constant"] = name
            entry["nearest_constant_value"] = const_val
    ...
    def _emergent_lambda(
        ...
        Misst wann lokale Strukturregeln globale Muster erzeugen die
        nicht explizit in den Eingabedaten enthalten waren — analog zu
        Conway's Game of Life: wenige Regeln, emergente Komplexitaet.

        Komponenten:
          0.40 — Symmetriebruch (globale vs. lokale Abweichung)
          0.40 — Kohaerenz-Ueberraschung (hohe Entropie + hohe Kohaerenz)
          0.20 — Integritaets-Phasenwechsel (STRUCTURAL_ANOMALY)

        Labels:  LATENT < 0.15 | EMERGING 0.15–0.35 | ACTIVE 0.35–0.60 | CRITICAL > 0.60
        """
        ...
        coherence_surprise = entropy_norm * coherence_norm
        phase_signal = 0.0
        if str(integrity_state) == "STRUCTURAL_ANOMALY":
            phase_signal = 1.0
        elif str(integrity_state) == "STRUCTURAL_TENSION":
            phase_signal = 0.5
        e_lambda = max(0.0, min(1.0,
            (0.40 * symmetry_break) + (0.40 * coherence_surprise) + (0.20 * phase_signal)
        ))
        if e_lambda < 0.15:
            label = "LATENT"
        elif e_lambda < 0.35:

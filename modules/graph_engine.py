...
def compute_stability(adjacency_matrix: list) -> float:
    """Berechnet Stabilitaet: 1 / (1 + Varianz der Knotengrade)"""
    if not adjacency_matrix:
        return 1.0
    degrees = [sum(row) for row in adjacency_matrix]
    var = _statistics.variance(degrees) if len(degrees) > 1 else 0.0
    return 1.0 / (1.0 + var)


def find_attractors(adjacency_matrix: list) -> list:
    """Finde Knoten mit Self-Loop (i==j). Gibt Liste der Indizes zurueck."""
    if not adjacency_matrix:
        return []

...
from .repositories import ChainRepository, DeltaRepository, UserRepository, VaultRepository
...
def is_genesis_node(node_id: str) -> bool:
    """Returns True iff node_id matches the cryptographically registered genesis node.

    Loads trusted_publishers.json and verifies:
    1. The genesis publisher entry exists and is enabled.
    2. The entry carries role == "genesis".
    3. The stored node_id matches the passed node_id (constant-time compare).

    This is the ONLY authoritative check for admin/genesis privileges.
    A matching role string alone is never sufficient.
    """
    if not str(node_id or "").strip():
        return False
    try:
        raw = TRUSTED_PUBLISHERS_PATH.read_text(encoding="utf-8")
        manifest = json.loads(raw)
        entry = (manifest.get("publishers") or {}).get(GENESIS_PUBLISHER_ID) or {}
        if not bool(entry.get("enabled", False)):
            return False
        if str(entry.get("role", "")).strip().lower() != "genesis":
            return False
        stored_id = str(entry.get("node_id", "") or "").strip()
        if not stored_id:
            return False
        # Constant-time comparison to prevent timing attacks
        import hmac as _hmac
        return _hmac.compare_digest(stored_id, str(node_id).strip())
    except Exception:
        return False
...
class AetherRegistry:
    ...
    def __init__(self, db_path: str) -> None:
        ...
        self.chat_registry = ChatRegistry(self.connection, now_iso=self._now_iso)
        self._create_tables()
        self.user_repository = UserRepository(
            self.connection,
            now_iso=self._now_iso,
            new_sync_materials=self._new_sync_materials,
            ensure_user_sync_material=self._ensure_user_sync_material,
            protect_local_secret=protect_local_secret,
        )
        self.vault_repository = VaultRepository(self.connection)
        self.chain_repository = ChainRepository(self.connection, now_iso=self._now_iso)
    ...
    def _create_tables(self) -> None:
        ...
        self.chat_registry._create_tables()
    ...
    def save(
        ...
        cursor = self.connection.execute(
            ...
            (
                ...
                self._pack_delta(fingerprint.delta, session_seed=int(session_context.seed)),
    ...
    def save_chain_block(
        ...
        return self.chain_repository.save_chain_block(
    ...
    def append_chain_block_annotation(
        ...
        return self.chain_repository.append_chain_block_annotation(
            block_id=block_id,
            session_id=session_id,
    ...
    def _latest_chain_annotations(self, block_ids: list[int]) -> dict[int, dict[str, Any]]:
        ...
        return self.chain_repository.latest_chain_annotations(block_ids)
    ...
    def get_chain_blocks_raw(
        ...
        return self.chain_repository.get_chain_blocks_raw(
            limit=limit,
            user_id=user_id,
    ...
    def get_chain_blocks(
        ...
        return self.chain_repository.get_chain_blocks(
            limit=limit,
            user_id=user_id,
            include_genesis=include_genesis,
        )
    ...
    def update_chain_block_payload(
        ...
        self.chain_repository.update_chain_block_payload(
            block_id=block_id,
            payload=payload,
            signature=signature,
        )
    ...
    def save_vault_entry(
        ...
        return self.vault_repository.save_vault_entry(
            session_id=session_id,
            source_type=source_type,
            source_label=source_label,
            file_hash=file_hash,
    ...
    def update_vault_cluster(self, entry_id: int, cluster_label: str) -> None:
        ...
        self.vault_repository.update_vault_cluster(entry_id, cluster_label)
    ...
    def update_vault_payload(
        ...
        self.vault_repository.update_vault_payload(
            entry_id=entry_id,
            payload=payload,
            signature=signature,
        )
    ...
    def get_vault_entries(self, limit: int = 300, user_id: int | None = None) -> list[dict[str, Any]]:
        ...
        return self.vault_repository.get_vault_entries(limit=limit, user_id=user_id)
    ...
    def save_delta_log(
        ...
        return self.delta_repository.save_delta_log(
            session_id=session_id,
            source_label=source_label,
            payload=payload,
            signature=signature,
    ...
    def get_delta_logs(self, session_id: str | None = None, limit: int = 500) -> list[dict[str, Any]]:
        ...
        return self.delta_repository.get_delta_logs(session_id=session_id, limit=limit)
    ...
    def reconstruct_original(
        ...
        delta = self._unpack_delta(bytes(row["delta"]), session_seed=seed)
    ...
    def has_users(self) -> bool:
        ...
        return self.user_repository.has_users()
    ...
    def update_user_security_state(
        ...
        self.user_repository.update_user_security_state(
            user_id=user_id,
    ...
    def update_user_settings(self, user_id: int, settings_update: dict[str, Any]) -> dict[str, Any]:
        ...
        return self.user_repository.update_user_settings(user_id, settings_update)
    ...
    def save_security_event(
        ...
        return self.user_repository.save_security_event(
            user_id=user_id,
            username=username,
    ...
    def get_security_events(self, limit: int = 120, user_id: int | None = None) -> list[dict[str, Any]]:
        ...
        return self.user_repository.get_security_events(limit=limit, user_id=user_id)
    ...
    def save_gp_rule_snapshot(
        ...
        return self.user_repository.save_gp_rule_snapshot(
            session_id=session_id,
            scope=scope,
            rule_type=rule_type,
            payload=payload,
    ...
    def get_gp_rule_snapshots(
        ...
        return self.user_repository.get_gp_rule_snapshots(
            limit=limit,
            scope=scope,
            include_honeypots=include_honeypots,
        )
    ...
    def open_user_session(
        ...
        return self.user_repository.open_user_session(
            session_id=session_id,
            user_id=user_id,
            username=username,
            role=role,
            ...
            live_key_fingerprint=live_key_fingerprint,
            algo_primary=algo_primary,
            algo_secondary=algo_secondary,
            payload=payload,
        )

    def close_user_session(self, session_id: str) -> None:
        """Markiert eine Login-Session sauber als beendet."""
        self.user_repository.close_user_session(session_id)
    ...
    def record_chat_sync_event(
        ...
        return self.user_repository.record_chat_sync_event(
            event_uid=event_uid,
            event_type=event_type,
    ...
    def has_chat_sync_event(self, event_uid: str) -> bool:
        ...
        return self.user_repository.has_chat_sync_event(event_uid)

    def get_chat_sync_cursor(self, endpoint: str) -> int:
        """Liefert die letzte bekannte Remote-ID fuer ein Relay-Ende."""
        return self.user_repository.get_chat_sync_cursor(endpoint)
    ...
    def update_chat_sync_cursor(self, endpoint: str, last_event_id: int) -> None:
        ...
        self.user_repository.update_chat_sync_cursor(endpoint, last_event_id)
    ...
    def export_user_sync_records(self, limit: int = 400) -> list[dict[str, Any]]:
        ...
        return self.user_repository.export_user_sync_records(limit=limit)
    ...
    def build_dna_share_snapshot(
        ...
        for entry in shared_entries:
            ...
            pattern_positions = [
                ...
                if str(anchor.get("nearest_constant", "")).upper() == "REF_A"
                or str(anchor.get("type_label", "")).upper() == "REF_A_LIKE"
            ][:8]
...
def register_module(name: str, module: object) -> None:
    """Speichert Modulreferenz in dict, ueberschreibt nicht ohne Warnung."""
    global _module_registry
    if name in _module_registry:
        print(f"Warnung: Modul '{name}' wird ueberschrieben.")
    _module_registry[name] = module


def get_module(name: str) -> object:
    """Gibt Modul aus dict oder None zurueck."""

...
| Boltzmann entropy | H_shannon / 8.0, normalized to [0,1] | Thermodynamic analogue |
| Zipf α | Power-law fit f ∝ r^−α | Token-frequency distribution shape |
| Benford score | Deviation from log₁₀(1+1/d) | Leading-digit naturalness in numeric data |
...
| Delta convergence | Euclidean distance to previous cascade run | Structural stability over time |
| Noether consistency | Forward/backward entropy symmetry | Symmetry preservation; breaks measurably on manipulation |
| Trust score | Composite of metrics 1–9 | Swarm participation gate: ≥ 0.65 required |
...
- The Noether consistency metric measures forward/backward byte-entropy symmetry. It is a
  concrete statistical computation, not a claim about Emmy Nöther's conservation theorems
  in physics. The name reflects the structural analogy (symmetry → preserved quantity), not
  a physico-mathematical equivalence.
